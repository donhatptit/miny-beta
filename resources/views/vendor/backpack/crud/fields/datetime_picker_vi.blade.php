...
if(isset($field['value'])){
            preg_match_all('/(\d+)/', $field['value'], $matches);
            $array_date = array_get($matches, 0, null);
            $str_date = '';
            if(!empty($array_date)){
                $day = array_get($array_date, 0, 0);
                $month = array_get($array_date, 1, 0);
                $year = array_get($array_date, 2, 0);
                $str_date = $year . '-' . $month . '-' . $day;
            }
            $field['value'] = $str_date;
        }
            ...
            $('[data-bs-datetimepicker]').each(function(){
                ...
                $picker.on('dp.change', function(e){
                    var sqlDate = e.date ? e.date.format('YYYY-MM-DD') : null;
                     arr_date = sqlDate.split('\-');
                     string_date = 'Ngày ' + arr_date[2] + ' tháng ' + arr_date[1] + ' năm ' + arr_date[0];
                     // console.log(string_date);
                    $field.val(string_date);
                });
