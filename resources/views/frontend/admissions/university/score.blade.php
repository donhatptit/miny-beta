...
@section('css')
    <style>
        {!! file_get_contents(public_path("frontend/css/university_score.css")) !!}
    </style>
@endsection
@section('content')
    <div class="post-heading" style="margin-bottom: 0px;">
        <div class="container">
            ...
            @php
                $breadcrumbs_render = Breadcrumbs::render('thong-tin-truong', $university, 'Điểm chuẩn');
            @endphp
            {{ $breadcrumbs_render or ''}}
            <div class="text-center">
                <h1 class="post-title" style="margin-bottom:0">Điểm chuẩn chính thức {{ $university->vi_name }} 2019 chính xác nhất.</h1>
            </div>
        </div>
    </div>
            ...
            <div class="container p-0 menu">
                <div class="col-md-8 ml-auto">
                    <ul class="list-unstyled list-inline list-menu text-center">
                        ...
                        <li class="list-inline-item item-active"><a href="{{ route('university.score', ['slug' => $university->slug]) }}">Điểm chuẩn</a></li>
        ...
        <div class="row detail-content">
            ...
            <div class="col-lg-8 university-content">
                <div class="card card-post-detail">
                            ...
                            <form class="form-inline mt-3" method="POST" action="{{ route('university.post.storeStatus') }}">
                                ...
                                <div class="form-group mb-2">
                                    ...
                                    <select name="status" class="form-control">
                                        <option value="1" {{ $score->is_approve == 1 ? "selected" : "" }}>Đã duyệt</option>
                                        <option value="0" {{ $score->is_approve == 0 ? "selected" : "" }}>Chưa duyệt</option>
                                    </select>
                                    <input type="hidden" name="id_post" value="{{ $score->id }}">
                    ...
                    <div class="card-body">
                        @if(count($years) > 0)
                        <h3 class="text-center" style="font-size:20px; color:#14a9e3">Điểm chuẩn {{ $university->vi_name }} năm {{ $year }}</h3>
                        <div class="ml-auto mb-1" style="width: 90px">
                            <form>
                        ...
                        @endif
                    @if(!empty($score->content))
                        {!! $score->content !!}
                        @else
                    <p class="text-center">Chưa có điểm chuẩn cho trường bạn yêu cầu, chúng tôi sẽ cập nhật sớm nhất...</p>
                        @endif
                    </div>
                </div>
